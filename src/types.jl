...
function adjusthour(h::Int128, ampm::AMPM)
    ampm == TWENTYFOURHOUR && return h
    ampm == PM && h < 12 && return h + 12
    ampm == AM && h == 12 && return Int128(0)
    return h
end
...
Construct a `DateTime` type by parts. Arguments must be convertible to [`Int128`](@ref).
"""
function DateTime(y::Int128, m::Int128=1, d::Int128=1,
                  h::Int128=0, mi::Int128=0, s::Int128=0, ms::Int128=0, ampm::AMPM=TWENTYFOURHOUR)
...
function validargs(::Type{DateTime}, y::Int128, m::Int128, d::Int128,
                   h::Int128, mi::Int128, s::Int128, ms::Int128, ampm::AMPM=TWENTYFOURHOUR)
...
Construct a `Date` type by parts. Arguments must be convertible to [`Int128`](@ref).
"""
function Date(y::Int128, m::Int128=1, d::Int128=1)
    err = validargs(Date, y, m, d)
    err === nothing || throw(err)
    return Date(UTD(totaldays(y, m, d)))
end

function validargs(::Type{Date}, y::Int128, m::Int128, d::Int128)
    0 < m < 13 || return argerror("Month: $m out of range (1:12)")
    0 < d < daysinmonth(y, m) + 1 || return argerror("Day: $d out of range (1:$(daysinmonth(y, m)))")
    return argerror()
end
...
Construct a `Time` type by parts. Arguments must be convertible to [`Int128`](@ref).
"""
function Time(h::Int128, mi::Int128=0, s::Int128=0, ms::Int128=0, us::Int128=0, ns::Int128=0, ampm::AMPM=TWENTYFOURHOUR)
...
function validargs(::Type{Time}, h::Int128, mi::Int128, s::Int128, ms::Int128, us::Int128, ns::Int128, ampm::AMPM=TWENTYFOURHOUR)
    if ampm == TWENTYFOURHOUR # 24-hour clock
...
DateTime(y, m=1, d=1, h=0, mi=0, s=0, ms=0, ampm::AMPM=TWENTYFOURHOUR) = DateTime(Int128(y), Int128(m), Int128(d), Int128(h), Int128(mi), Int128(s), Int128(ms), ampm)
Date(y, m=1, d=1) = Date(Int128(y), Int128(m), Int128(d))
Time(h, mi=0, s=0, ms=0, us=0, ns=0, ampm::AMPM=TWENTYFOURHOUR) = Time(Int128(h), Int128(mi), Int128(s), Int128(ms), Int128(us), Int128(ns), ampm)
