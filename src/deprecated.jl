...
function (+)(x::AbstractArray{<:TimeType}, y::GeneralPeriod)
    # depwarn("non-broadcasted arithmetic is deprecated for DatesPlus.TimeType; use broadcasting instead", nothing)
    x .+ y
end
...
function (+)(x::StridedArray{<:GeneralPeriod})
    # depwarn("non-broadcasted operations are deprecated for DatesPlus.TimeType; use broadcasting instead", nothing)
    x
end
