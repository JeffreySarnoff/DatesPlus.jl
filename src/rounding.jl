...
corresponding `Date`.
"""
epochdays2date(i) = Date(UTD(DATEEPOCH + Int128(i)))
...
"""
epochms2datetime(i) = DateTime(UTM(DATETIMEEPOCH + Int128(i)))

"""
    date2epochdays(dt::Date) -> Int128

Take the given `Date` and return the number of days since the rounding epoch
(`0000-01-01T00:00:00`) as an [`Int128`](@ref).
"""
date2epochdays(dt::Date) = value(dt) - DATEEPOCH

"""
    datetime2epochms(dt::DateTime) -> Int128
...
function Base.floor(dt::Date, p::Week)
    ...
    return Date(UTD(WEEKEPOCH + Int128(days)))
end
