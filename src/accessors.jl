...
hour(t::Time)   = mod(fld(value(t), 3600000000000), Int128(24))
minute(t::Time) = mod(fld(value(t), 60000000000), Int128(60))
second(t::Time) = mod(fld(value(t), 1000000000), Int128(60))
millisecond(t::Time) = mod(fld(value(t), Int128(1000000)), Int128(1000))
microsecond(t::Time) = mod(fld(value(t), Int128(1000)), Int128(1000))
nanosecond(t::Time) = mod(value(t), Int128(1000))
...
for func in (:year, :month, :quarter)
    ...
    @eval begin
        @doc """
            $($name)(dt::TimeType) -> Int128

        The $($name) of a `Date` or `DateTime` as an [`Int128`](@ref).
        """ $func(dt::TimeType)
    end
end

"""
    week(dt::TimeType) -> Int128

Return the [ISO week date](https://en.wikipedia.org/wiki/ISO_week_date) of a `Date` or
`DateTime` as an [`Int128`](@ref). Note that the first week of a year is the week that
...
# Examples
```jldoctest
julia> DatesPlus.week(Date(1989, 6, 22))
25

julia> DatesPlus.week(Date(2005, 1, 1))
53

julia> DatesPlus.week(Date(2004, 12, 31))
53
```
"""
week(dt::TimeType)
...
for func in (:day, :dayofmonth)
    ...
    @eval begin
        ...
        The day of month of a `Date` or `DateTime` as an [`Int128`](@ref).
        """ $func(dt::TimeType)
    end
end

"""
    hour(dt::DateTime) -> Int128

The hour of day of a `DateTime` as an [`Int128`](@ref).
"""
hour(dt::DateTime)
...
for func in (:minute, :second, :millisecond)
    ...
    @eval begin
        @doc """
            $($name)(dt::DateTime) -> Int128

        The $($name) of a `DateTime` as an [`Int128`](@ref).
        """ $func(dt::DateTime)
    end
end
...
for parts in (["year", "month"], ["month", "day"], ["year", "month", "day"])
    ...
    @eval begin
        @doc """
            $($name)(dt::TimeType) -> ($(join(repeated(Int128, length($parts)), ", ")))
...
for func in (:hour, :minute, :second, :millisecond, :microsecond, :nanosecond)
    ...
    @eval begin
        ...
        The $($name) of a `Time` as an [`Int128`](@ref).
        """ $func(t::Time)
    end
end
