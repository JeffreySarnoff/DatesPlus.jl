...
for period in (:Year, :Quarter, :Month, :Week, :Day, :Hour, :Minute, :Second, :Millisecond, :Microsecond, :Nanosecond)
    ...
    @eval $period(x::AbstractString) = $period(Base.parse(Int128, x))
    ...
    @eval begin
        ...
        losslessly convertible to an [`Int128`](@ref).
        """ $period(v)
    end
end
...
Base.typemin(::Type{P}) where {P<:Period} = P(typemin(Int128))
Base.typemax(::Type{P}) where {P<:Period} = P(typemax(Int128))
...
div(x::P, y::Real, r::RoundingMode) where {P<:Period} = P(div(value(x), Int128(y), r))

for op in (:rem, :mod)
    @eval begin
        ($op)(x::P, y::P) where {P<:Period} = P(($op)(value(x), value(y)))
        ($op)(x::P, y::Real) where {P<:Period} = P(($op)(value(x), Int128(y)))
    end
end
...
for i = 1:length(fixedperiod_conversions)
    ...
    for j = (i - 1):-1:1 # less-precise periods
        ...
        N *= nc
        vmax = typemax(Int128) ÷ N
        vmin = typemin(Int128) ÷ N
...
let vmax = typemax(Int128) ÷ 12, vmin = typemin(Int128) ÷ 12
...
let vmax = typemax(Int128) ÷ 4, vmin = typemin(Int128) ÷ 4
...
let vmax = typemax(Int128) ÷ 3, vmin = typemin(Int128) ÷ 3
...
const zero_or_fixedperiod_seed = UInt === UInt128 ? 0x5b7fc751bba97516 : 0xeae0fdcb
const nonzero_otherperiod_seed = UInt === UInt128 ? 0xe1837356ff2d2ac9 : 0x170d1b00
