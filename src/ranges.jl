...
guess(a::DateTime, b::DateTime, c) = floor(Int128, (Int128(value(b)) - Int128(value(a))) / toms(c))
guess(a::Date, b::Date, c) = Int128(div(value(b - a), days(c)))
len(a::Time, b::Time, c) = Int128(div(value(b - a), tons(c)))
function len(a, b, c)
...
Base.length(r::StepRange{<:TimeType}) = isempty(r) ? Int128(0) : len(r.start, r.stop, r.step) + 1
# Period ranges hook into Int128 overflow detection
